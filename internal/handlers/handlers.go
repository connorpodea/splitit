...
import (
	...
	"github.com/you/p2p-bnpl/internal/bnpl"
...
func (h *Handler) handleUsers(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		if err := decode(r, &u); err != nil {
			httpErr(w, err.Error(), 400); return
		}
		if u.ID == "" || u.Name == "" || u.Email == "" {
			httpErr(w, "id, name, email required", 400); return
		}
...
func (h *Handler) handleUserByID(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { httpErr(w, err.Error(), 404); return }
	respond(w, u)
}
...
func (h *Handler) handlePayments(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		if p.ID == "" || p.SenderID == "" || p.ReceiverID == "" || p.Amount <= 0 {
			httpErr(w, "id, sender_id, receiver_id, amount required", 400); return
		}
		if p.SenderID == p.ReceiverID {
			httpErr(w, "sender and receiver must be different users", 400); return
		}

		// Make sure both users exist
		if _, err := h.store.GetUser(p.SenderID); err != nil {
			httpErr(w, "sender not found", 404); return
		}
		if _, err := h.store.GetUser(p.ReceiverID); err != nil {
			httpErr(w, "receiver not found", 404); return
		}

		if err := h.store.CreatePayment(p); err != nil {
			httpErr(w, err.Error(), 500); return
		}
		...
		if p.Method == models.MethodBNPL {
			...
			if err != nil { httpErr(w, err.Error(), 400); return }
...
func (h *Handler) handlePlans(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		httpErr(w, "method not allowed", 405); return
	}
...
func (h *Handler) handlePayInstallment(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) != 2 {
		httpErr(w, "path: /api/pay/{planID}/{number}", 400); return
	}
	planID := parts[0]
	n, err := parseInt(parts[1])
	if err != nil { httpErr(w, "installment number must be an integer", 400); return }
